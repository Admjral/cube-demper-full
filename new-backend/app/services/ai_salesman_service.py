...
_KZ_CHARS = re.compile(r'[ӘәҒғҚқҢңӨөҰұҮүҺһІі]')


def detect_language(text: str) -> str:
    """
    Определить язык сообщения: 'kz' или 'ru'.

    Если в тексте есть специфические казахские буквы → 'kz'.
    Иначе → 'ru' (дефолт).
    """
    if _KZ_CHARS.search(text):
        return 'kz'
    return 'ru'
...
class AISalesmanService:
    ...
    SYSTEM_PROMPTS = {
        SalesmanTrigger.NEW_ORDER: """Ты ИИ продажник для маркетплейса в Казахстане. Твоя задача - составить короткое, дружелюбное сообщение покупателю в WhatsApp с предложением дополнительных товаров.

Правила:
1. Отвечай НА ТОМ ЖЕ ЯЗЫКЕ, на котором указано имя клиента или на языке, указанном в промпте (русский или казахский)
2. Максимум 3-4 предложения
3. Упомяни 1-2 релевантных товара из каталога
4. Не используй эмодзи больше 1-2 штук
5. Обращайся на "вы" (русский) или "Сіз" (казахский)
6. Не предлагай скидки и промокоды
7. Не обещай того, чего нет в каталоге

ВАЖНО: Ответ должен быть НЕ ДЛИННЕЕ 280 символов. Это жёсткий лимит — сообщения длиннее будут обрезаны.

Формат ответа:
Только текст сообщения, без пояснений.""",

        SalesmanTrigger.REPEAT_CUSTOMER: """Ты ИИ продажник для маркетплейса в Казахстане. Покупатель делает повторную покупку - это важный клиент!
...
Правила:
- Отвечай НА ТОМ ЖЕ ЯЗЫКЕ, на котором указано имя клиента или на языке, указанном в промпте (русский или казахский)
- Максимум 4 предложения
- Дружелюбный тон
- Без навязчивости
- Обращайся на "вы" (русский) или "Сіз" (казахский)
...
Только текст сообщения.""",
        ...
        SalesmanTrigger.REVIEW_REQUEST: """Ты ИИ помощник для маркетплейса в Казахстане. Твоя задача - вежливо попросить покупателя оставить отзыв.

Правила:
1. Отвечай НА ТОМ ЖЕ ЯЗЫКЕ, на котором указано имя клиента или на языке, указанном в промпте (русский или казахский)
2. 2-3 предложения максимум
3. Не давить, просто напомнить
4. Обращайся на "вы" (русский) или "Сіз" (казахский)

ВАЖНО: Ответ должен быть НЕ ДЛИННЕЕ 160 символов. Это жёсткий лимит — сообщения длиннее будут обрезаны.
...
Только текст сообщения.""",
    ...
    async def generate_upsell_message(
        ...
        try:
            ...
            # Fallback на шаблон (определяем язык по имени клиента)
            lang = detect_language(context.customer_name or '')
            return self._get_fallback_message(context, trigger, lang=lang)
...
Составь короткое сообщение для отправки в WhatsApp."""
    ...
    def _get_fallback_message(
        ...
        name_part_ru = f", {context.customer_name}" if context.customer_name else ""
        name_part_kz = f", {context.customer_name}" if context.customer_name else ""

        if trigger == SalesmanTrigger.REPEAT_CUSTOMER:
            if lang == 'kz':
                text = f"Сәлеметсіз бе{name_part_kz}! Қайта тапсырыс бергеніңізге рахмет. Сіздің адалдығыңызды бағалаймыз! Тауар таңдауда көмек қажет болса — жазыңыз."
            else:
                text = f"Здравствуйте{name_part_ru}! Спасибо за повторный заказ. Мы ценим вашу лояльность! Если нужна помощь с выбором товаров - пишите."
        elif trigger == SalesmanTrigger.REVIEW_REQUEST:
            if lang == 'kz':
                text = "Сәлеметсіз бе! Тауар ұнады деп сенеміз. Пікір қалдырсаңыз — өте ризашылық білдірер едік!"
            else:
                text = "Здравствуйте! Надеемся, вам понравился товар. Будем благодарны за отзыв - это помогает другим покупателям."
        else:
            if lang == 'kz':
                text = f"Сәлеметсіз бе{name_part_kz}! Тапсырысыңызға рахмет. Қосымша тауар қажет болса — көмектесуге дайынбыз!"
            else:
                text = f"Здравствуйте{name_part_ru}! Спасибо за заказ. Если понадобятся дополнительные товары - будем рады помочь!"
...
Составь вежливую просьбу оставить отзыв."""
        ...
        try:
            ...
            lang = detect_language(context.customer_name or '')
            return self._get_fallback_message(context, SalesmanTrigger.REVIEW_REQUEST, lang=lang)
...
CHAT_SYSTEM_PROMPT = """Ты ИИ-продажник для магазина на маркетплейсе Kaspi.kz (Казахстан). Клиент написал тебе в WhatsApp. Твоя задача — помочь клиенту и при уместности предложить товары из каталога магазина.

Правила:
1. Отвечай НА ТОМ ЖЕ ЯЗЫКЕ, на котором написал клиент (русский или казахский)
...
9. Обращайся на "вы" (русский) или "Сіз" (казахский)

ВАЖНО: Ответ должен быть НЕ ДЛИННЕЕ 350 символов. Это жёсткий лимит — сообщения длиннее будут обрезаны.
...
Составь ответ."""
        ...
        try:
            ...
            if lang == 'kz':
                reply_text = "Сәлеметсіз бе! Хабарламаңызға рахмет. Менеджеріміз жақын арада сізбен байланысады."
            else:
                reply_text = "Здравствуйте! Спасибо за сообщение. Наш менеджер свяжется с вами в ближайшее время."
...
async def _generate_upsell_reply(
    ...
    system_prompt = """Ты ИИ-продажник магазина на Kaspi.kz (Казахстан). Клиент оставил отзыв о завершенном заказе.
...
Правила:
- Отвечай НА ТОМ ЖЕ ЯЗЫКЕ, на котором написал клиент (русский или казахский)
- На "вы" (русский) или "Сіз" (казахский)
...
Твой ответ:"""
    ...
    try:
        ...
        response = await model.generate_content_async(
            user_prompt,
            generation_config=genai.GenerationConfig(max_output_tokens=800, temperature=0.7)
        )
        return response.text.strip()
    except Exception as e:
        logger.error(f"Gemini upsell failed: {e}")
        # Fallback — определяем язык по сообщению клиента
        lang = detect_language(customer_message)
        if products:
            prod = products[0]
            if lang == 'kz':
                return f"Пікіріңізге рахмет! Айтпақшы, бізде {prod['name']} {prod['price']}₸ бар, тапсырысыңызды тамаша толықтырады."
            else:
                return f"Спасибо за отзыв! Кстати, у нас есть {prod['name']} за {prod['price']}₸, отлично дополнит ваш заказ."
        else:
            if lang == 'kz':
                return "Пікіріңізге рахмет! Тағы бірдеңе қажет болса — жазыңыз!"
            else:

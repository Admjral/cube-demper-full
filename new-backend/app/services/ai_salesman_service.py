...
Использует OpenAI для генерации персонализированных сообщений
...
Только текст сообщения.""",
    ...
    def __init__(self, openai_api_key: str = None):
        ...
        Args:
            openai_api_key: API ключ OpenAI (или из settings)
        """
    ...
    async def generate_upsell_message(
        ...
        try:
            ...
            response = await self.openai_client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": self.SYSTEM_PROMPTS[trigger]},
...
Составь короткое сообщение для отправки в WhatsApp."""
    ...
    def _get_fallback_message(
        ...
        """Получить fallback сообщение если OpenAI недоступен"""
...
Составь вежливую просьбу оставить отзыв."""
        ...
        try:
            response = await self.openai_client.chat.completions.create(
                model="gpt-3.5-turbo",
                messages=[
                    {"role": "system", "content": self.SYSTEM_PROMPTS[SalesmanTrigger.REVIEW_REQUEST]},
                    {"role": "user", "content": user_prompt},
                ],
                max_tokens=200,
                temperature=0.7,
